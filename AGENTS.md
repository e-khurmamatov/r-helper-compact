...
- Device identity must fail closed in the controller as well as the UI. Require Razer system manufacturer and a valid RZ09 chassis SKU; USB VID alone does not identify the host. Reviewed command profiles retain exact PID/SKU matching. Other Razer laptops may use explicitly labeled experimental Discovery only after one Blade HID controller is identified by USB product metadata. Reject missing host identity and ambiguous controllers. Disabled/candidate records must not supply model-specific command profiles; external support lists are identity evidence, not hardware verification.
...
- Do not commit, package or publish additional agent-generated Markdown files unless they are explicitly required. Store local research, plans, handoffs and other agent artifacts under ignored `work/agent-artifacts/`. Update existing project documentation when the task requires it; keep necessary device provenance in the device records.
